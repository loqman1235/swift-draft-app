  ...
  return (
    <ul className="flex h-[calc(100vh-var(--navbar-height))] flex-col gap-1 px-2.5 py-5">
      {dashboardRoutes.map((route) => (
        <li key={route.href}>
          <Link
            className={`text-text-muted flex items-center gap-5 rounded-lg px-3 py-4 transition hover:bg-primary/10 hover:text-primary ${pathname === route.href && activeClass}`}
            href={route.href}
          >
      ...
      <li className="mt-auto">
        <button className="text-text-muted flex w-full items-center gap-5 rounded-lg px-3 py-4 transition hover:bg-primary/10 hover:text-primary">
